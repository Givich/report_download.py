...
def process_reports(sess, task, output_dir, dry_run):
    ...
    for report in res.json()['reports']['reports']:
        ...
        if not report['filelink']:
            continue
        path_temp = os.path.join(output_dir, task['subject_name'][0], report['group_num'])
        #print(task)
        if task['tt_name'] == 'Курсовой проект (работа)':
        ...
        # os.makedirs(path, exist_ok=True)
        # ispath = os.path.join(output_dir, task['subject_name'][0], report['group_num'])
        ...
        if report['status_name'] != 'не принят':
...
def main():
    ...
    if params.username:
        ...
        # authorize
        res = sess.post('https://pro.guap.ru/user/login_check',
                        data={'_username': params.username, '_password': password})
        ...
        logger.error(
            "No authentication details were provided. Please, specify a user name or a PHPSESSID cookie value for pro.guap.ru as a command line parameter for this script")
        exit(1)
    ...
    if params.impersonate:
        ...
        if res.status_code != 200:
            logger.error("Unable to impersonate user '%s'. Server response: %s (%d)", params.impersonate, res.reason,
                         res.status_code)
    ...
    for task in res.json()['tasks']:
        if task['hasReports']:
            subject = task['subject_name'][
                0]  # don't bother with different subject names for different groups, just use the first one
            # print(task)
    ...
    if params.batch:
        for task_id in tasks:
            if task['hasReports']:
                logger.info("Processing reports for task '%s: %s'", tasks[task_id]['subject_name'][0],
                            tasks[task_id]['name'])
                process_reports(sess, tasks[task_id], params.output_dir, params.dry_run)
    else:
        print("\n".join([
            f"{task['id']}: {task['subject_name'][0]}: {task['name']} ({groups}) [Студентов: {task['count']}; принято: {task['doneWork']}, ожидают: {task['await']}]"
            for task in res.json()['tasks'] if task['hasReports']
        ]))
        print()
